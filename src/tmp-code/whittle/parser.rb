...
module Whittle
  class Parser
    class << self
      ...
      # 声明一条规则
      #
      # 终结符用rule("string")或rule(symbol: /regexp/)
      # 非终结符用rule(:symbol){|r| r[:sym, ...].as{|a,b,...| expr}; ... }，非终结符可以衍生多条规则
      #
      # @param [Symbol, String, Hash] name
      #   字符串或符号指向正则表达式的散列
      #
      # @return [RuleSet, Rule]
      #   根据给定块创建规则集，如无处理块则表现为终结符标识
      def rule(name)
        if block_given?
          raise ArgumentError, "Parser#rule does not accept both a Hash and a block" if name.kind_of?(Hash)
          rules[name] = RuleSet.new(name, false)
          rules[name].tap { |r| yield r }
        else
          key, value = if name.kind_of?(Hash)
            raise ArgumentError, "Only one element allowed in Hash for Parser#rule" unless name.length == 1
            name.first
          else
            [name, name]
          end
